...
const HostsForm: React.FC<HostsFormProps> = ({ control, handleChange }) => {
    ...
    const clusterDeployment = clusterDeployments.find((cd) => cd.metadata.name === cdName && cd.metadata.namespace === cdName);
    const agentClusterInstall = agentClusterInstalls.find((aci) => aci.metadata.name === aciName && aci.metadata.namespace === aciName);
