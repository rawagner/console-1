...
const AIHypershiftClusterDetails: React.FC = () => {
    ...
    const clusterNodePools = nodePools.filter((np) => np.metadata.namespace === hostedCluster?.metadata.namespace && np.spec.clusterName === hostedCluster?.metadata.name)
            ...
            <div style={{ marginBottom: '24px' }}>
                <AcmExpandableCard title="Cluster installation progress" id="aiprogress">
                    <ClusterInstallationProgress
                        ...
                        fetchSecret={(name, namespace) => getResource({ kind:'Secret', apiVersion:'v1', metadata: { name, namespace} }).promise}
                        nodePools={clusterNodePools}
                        clusterImages={clusterImageSets}
                        onRemoveNodePool={(np) => {
                            const agentsToPatch = agents?.filter((a) =>
                                a.metadata.labels?.['agentclusterinstalls.extensions.hive.openshift.io/nodePool'] === np.metadata.name
                                || a.metadata.labels?.['agentclusterinstalls.extensions.hive.openshift.io/nodePoolNs'] === np.metadata.namespace
                            )
                        ...
                        onUpdateNodePool={async (nodePool, newAgents) => {
                            const agentsToPatch = newAgents.filter((a) =>
                                a.metadata.labels?.['agentclusterinstalls.extensions.hive.openshift.io/nodePool'] !== nodePool.metadata.name
                                || a.metadata.labels?.['agentclusterinstalls.extensions.hive.openshift.io/nodePoolNs'] !== nodePool.metadata.namespace
                            )
                            const uids = newAgents.map((a) => a.metadata.uid)
                            const agentsToRemove = agents?.filter((a) =>
                                ...
                                patchResource(a, [
                                    {
                                        ...
                                        value: {
                                            ...(a.metadata.labels || {}),
                                            'agentclusterinstalls.extensions.hive.openshift.io/nodePool': nodePool.metadata.name,
                                            'agentclusterinstalls.extensions.hive.openshift.io/nodePoolNs': nodePool.metadata.namespace,
                                        },
                                    },
                                ]).promise
                            })
